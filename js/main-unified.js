...
class WoodcutterApp {
     ...
     */
    async init() {
        console.log('대산Ai V1 통합 버전 초기화 중...');
    ...
    bindEvents() {
        ...
        if (boardThickness) {
            // boardThickness 변경 시 cutPriceDisplay 업데이트 로직 제거 (수동 입력 우선)
        }
        if (considerGrain) {
        ...
        // Settings auto-save
        const settingInputs = ['kerfInput', 'enableTrim', 'trimMargin', 'cutDirection', 'cutPriceInput'];
    ...
    /**
     * 절단 단가 단가 업데이트 (수동 가격 입력 대응)
     */
    updateCutPriceDisplay() {
        const cutPriceInput = document.getElementById('cutPriceInput');
        if (!cutPriceInput) return;

        const cutPrice = parseFloat(cutPriceInput.value) || 1500;
